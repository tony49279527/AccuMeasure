...
- Local 390×844 isolated-browser verification passed for the corrected build: the flow page has no whole-page horizontal overflow, its comparison table retains an internal horizontal-scroll container, and the visible Quote CTA remains available. AM-RL80 has no whole-page horizontal overflow; all six optional configuration inputs and the Quote CTA are visible and enabled.
